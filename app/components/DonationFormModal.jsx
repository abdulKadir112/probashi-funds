      ...
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] p-5 md:p-8 relative shadow-2xl animate-in zoom-in duration-200 max-h-[92vh] flex flex-col border border-slate-100">
        ...
        {step !== 'receipt' && (
          <button 
            ...
            className="absolute top-6 right-6 p-2 bg-slate-50 text-slate-400 rounded-full hover:text-rose-500 transition-all shadow-sm"
          >
            <X size={18} />
          </button>
        )}
        ...
        {step === 'form' && (
          <>
            <div className="flex items-center gap-3 mb-5 border-b pb-3">
              <div className="w-10 h-10 bg-rose-600 text-white rounded-xl flex items-center justify-center shadow-md shadow-rose-100">
            ...
            <form onSubmit={handleGoToPreview} className="flex-1 overflow-y-auto pr-1 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="tel" required placeholder="017XXXXXXXX" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-slate-950 font-semibold rounded-xl text-xs focus:ring-2 focus:ring-rose-400 focus:outline-none" />
                  {errors.phone && <p className="text-[11px] text-rose-500 font-bold mt-1">{errors.phone}</p>}
                </div>

                <div>
                  <label className="text-xs font-black text-slate-700 mb-1 flex items-center gap-1"><CreditCard size={14} className="text-rose-500" /> এনআইডি (NID) *</label>
                  <input type="number" required placeholder="10/17 ডিজিট" value={formData.nid} onChange={(e) => setFormData({...formData, nid: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-slate-950 font-semibold rounded-xl text-xs focus:ring-2 focus:ring-rose-400 focus:outline-none" />
              ...
              <div className="bg-slate-50/60 p-4 rounded-2xl border border-slate-100 space-y-3">
                <p className="text-xs font-black text-slate-800 flex items-center gap-1"><MapPin size={14} className="text-rose-500" /> বর্তমান ঠিকানা</p>
              ...
              <div>
                <label className="text-xs font-black text-slate-700 mb-1 flex items-center gap-1"><DollarSign size={14} className="text-rose-500" /> অনুরোধকৃত টাকার পরিমাণ *</label>
                <input type="number" required placeholder="টাকার পরিমাণ" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-slate-950 font-black text-sm rounded-xl focus:ring-2 focus:ring-rose-400 focus:outline-none" />
                {errors.amount && <p className="text-[12px] text-rose-500 font-bold mt-1">{errors.amount}</p>}
              </div>

              <div>
                <label className="text-xs font-black text-slate-700 mb-1 flex items-center gap-1"><FileText size={14} className="text-rose-500" /> সহায়তার কারণ *</label>
                <textarea required rows={3} placeholder="বিস্তারিত কারণ লিখুন..." value={formData.reason} onChange={(e) => setFormData({...formData, reason: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-950 font-semibold rounded-xl text-xs focus:ring-2 focus:ring-rose-400 focus:outline-none resize-none" />
              </div>

              <button type="submit" className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white font-black rounded-xl flex items-center justify-center gap-2 text-xs uppercase tracking-wider">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 bg-slate-50 p-4 rounded-2xl border text-xs">
              <div className="grid grid-cols-2 gap-y-3 border-b pb-3">
                ...
                <p className="text-slate-500 font-bold">জন্ম তারিখ:</p><p className="font-black">{formData.dob}</p>
              </div>
              ...
              <div className="flex justify-between items-center bg-rose-50 p-3 rounded-xl border border-rose-100">
                <span className="font-black text-rose-700">টাকার পরিমাণ:</span>
            ...
            <div ref={printRef} className="flex-1 p-6 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
              ...
              <table style={{width:'100%'}}>
                <tbody>
                  ...
                  <tr><td>NID:</td><td className="bold right">{formData.nid}</td></tr>
              ...
              <p className="footer-text">স্লিপটি প্রিন্ট করে রাখুন। আমাদের টিম শীঘ্রই যোগাযোগ করবে।</p>
            </div>
