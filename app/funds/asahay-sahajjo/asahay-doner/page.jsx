...
import { ArrowLeft, Search, Users2, Award, History, X, CalendarDays, Wallet, Filter } from 'lucide-react';
...
function DonorsContent() {
  ...
  // শুধুমাত্র 'asahay-sahajjo' ফান্ডের ডাটা দেখানোর জন্য আইডি সেট করা
  const TARGET_FUND = 'asahay-sahajjo';
  ...
  // ট্রানজেকশন থেকে শুধুমাত্র 'asahay-sahajjo' এর দাতাদের প্রসেস করা
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-24">
      {/* টপ বার */}
      <div className="bg-white/80 backdrop-blur-md border-b border-indigo-50 sticky top-0 z-40 p-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <Link href={`/funds/${TARGET_FUND}`}>
              <button className="p-3 bg-white border border-indigo-100 text-indigo-600 rounded-2xl hover:bg-indigo-50 transition shadow-sm">
                <ArrowLeft size={20} />
              </button>
            </Link>
            <h2 className="text-xl font-black text-slate-800 italic">অসহায় সাহায্য দাতা তালিকা</h2>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:max-w-xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-2.5 bg-indigo-50/30 border border-indigo-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-400 text-sm font-medium"
              />
            </div>
            <div className="relative">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-400" size={14} />
              <select
                value={filterType}
                onChange={(e) => setFilterType(e.target.value)}
                className="pl-9 pr-4 py-2.5 bg-white border border-indigo-100 rounded-2xl text-xs font-black text-indigo-600 outline-none appearance-none cursor-pointer shadow-sm hover:bg-indigo-50"
              >
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-10">
        <div className="text-center mb-12">
          <div className="inline-block p-4 bg-gradient-to-br from-indigo-600 to-blue-700 text-white rounded-[2.5rem] shadow-xl shadow-indigo-100 mb-6">
            <Users2 size={40} />
          </div>
          ...
          <p className="text-indigo-600 font-black uppercase tracking-widest text-[10px]">অসহায় সাহায্য তহবিলে অবদান রাখছেন যারা</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {donors.map((donor, idx) => (
            <div 
              ...
              className="bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-200 transition-all duration-500 group cursor-pointer relative overflow-hidden"
            >
              ...
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-blue-600 text-white rounded-3xl flex items-center justify-center text-2xl font-black shadow-lg group-hover:scale-110 transition-transform">
              ...
              <div className="mt-6 flex items-center justify-between bg-slate-50 rounded-2xl p-4 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                <div>
                  <p className="text-[9px] uppercase font-black opacity-60 mb-1">মোট অবদান</p>
        ...
        {donors.length === 0 && (
          <div className="text-center py-24 bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
            ...
            <p className="text-slate-400 font-bold italic">এই তহবিলে কোনো দানকারীর রেকর্ড পাওয়া যায়নি</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-[3rem] p-6 md:p-10 relative shadow-2xl animate-in zoom-in duration-200 max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-4">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2 flex items-center gap-2">
                <History size={12} /> অনুদানের ইতিহাস (অসহায় সাহায্য)
              </p>
              ...
              {transactions
                ...
                .map((d, i) => (
                  <div key={i} className="bg-slate-50 rounded-3xl p-5 border border-slate-100 flex justify-between items-center group hover:bg-white hover:border-indigo-200 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-black text-slate-800 text-xs italic">
                           → {d.receiverName ? `সংগ্রহকারী: ${d.receiverName}` : fundNameMap[TARGET_FUND]}
                        </p>
                      </div>
                    </div>
            ...
            <div className="mt-8 pt-8 border-t border-slate-100 flex justify-between items-end">
              <div>
                <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest mb-1">তহবিলে মোট অবদান</p>
                <p className="text-4xl font-black text-slate-900 italic leading-none">৳ {selectedDonor.totalDonated.toLocaleString('bn-BD')}</p>
              </div>
              <div className="bg-indigo-50 px-5 py-2.5 rounded-2xl border border-indigo-100">
                <p className="text-indigo-600 font-black text-xs italic tracking-tighter uppercase">Asahay Sahajjo Fund</p>
              </div>
            </div>
          </div>
        </div>
