    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-10">
        <div className="text-center mb-10 md:mb-14">
          <div className="inline-block p-4 bg-rose-600 text-white rounded-[2rem] shadow-xl shadow-rose-100 mb-4 animate-pulse">
            <HeartHandshake size={32} />
          </div>
          <h1 className="text-3xl md:text-5xl font-black text-slate-900 mb-2 italic tracking-tight text-center">সাহায্য গ্রহীতা তালিকা</h1>
          <p className="text-rose-600 font-black uppercase tracking-widest text-[10px]">অসহায় সাহায্য তহবিল ২০২৬</p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8">
          {asahayReceivers.map((receiver, idx) => (
            <div 
              key={idx}
              onClick={() => setSelectedReceiver(receiver)}
              className="bg-white rounded-[2.5rem] p-5 md:p-8 shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300 group cursor-pointer relative overflow-hidden text-center"
            >
              <div className="w-16 h-16 md:w-24 md:h-24 bg-gradient-to-br from-rose-500 to-pink-600 rounded-[2rem] mx-auto mb-4 flex items-center justify-center text-white text-2xl md:text-4xl font-black shadow-lg">
                {getInitial(receiver.name)}
              </div>
              
              <h3 className="text-sm md:text-xl font-black text-slate-800 mb-2 truncate italic">{receiver.name}</h3>
              
              <div className="bg-slate-100 rounded-2xl p-2 md:p-5 group-hover:bg-rose-600 group-hover:text-white transition-all">
                <p className="text-[8px] md:text-[10px] text-slate-600 group-hover:text-rose-100 uppercase font-black mb-1">প্রদানকৃত অর্থ</p>
              ...
              <div className="mt-2">
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-tighter italic">বিস্তারিত দেখতে ক্লিক করুন</span>
              </div>
            </div>
          ))}
        </div>

        {asahayReceivers.length === 0 && !isLoading && (
          <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
             <UserCircle size={40} className="mx-auto text-slate-100 mb-4" />
             <p className="text-slate-400 font-bold italic">এই তহবিলে এখনো কোনো গ্রহীতার তথ্য নেই</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-6 md:p-10 relative shadow-2xl animate-in zoom-in duration-200 max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center gap-4 md:gap-6 mb-6 border-b pb-6">
              <div className="w-16 h-16 bg-rose-500 text-white rounded-[1.5rem] flex items-center justify-center text-2xl font-black shadow-md">
              ...
              <div>
                ...
                <span className="text-[10px] font-black text-rose-600 uppercase tracking-widest bg-rose-50 px-3 py-1 rounded-full border border-rose-100">সহায়তা গ্রহীতা</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-4">
              ...
              {selectedReceiver.history
                ...
                .map((t, i) => (
                  <div key={i} className="bg-slate-50 rounded-3xl p-5 border border-slate-100 shadow-sm group hover:bg-white hover:border-rose-200 transition-all">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center text-rose-500 shadow-sm border border-slate-50">
                          <UserCircle size={20} />
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xl font-black text-rose-600 italic leading-none">৳ {Number(t.amount).toLocaleString('bn-BD')}</p>
                      </div>
                    </div>
...
export default function AsahayReceiverPage() {
  return (
    <Suspense fallback={<IslamicLoader />}>
      <AsahayReceiverContent />
    </Suspense>
  ); 
}