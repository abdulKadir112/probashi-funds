...
import { useStore } from '../../../lib/store'; // আপনার প্রজেক্টের সঠিক পাথ অনুযায়ী এটি পরিবর্তন করে নিবেন

export default function EidgahDashboard() {
  const { 
    transactions, 
    fetchData, 
    addTransaction, 
  ...
  useEffect(() => {
    const savedAuth = localStorage.getItem('eidgah_admin_auth');
  ...
  // ================== শুধু eidgah-tohobil ফান্ডের ডেটা ==================
  ...
  const handleAdminLogin = (e) => {
    ...
    if (inputPasskey === ADMIN_PASSKEY) {
      setIsAuthorized(true);
      localStorage.setItem('eidgah_admin_auth', ADMIN_PASSKEY);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('eidgah_admin_auth');
    setInputPasskey('');
    setAllDonors([]);
  };
  ...
  const handleDonationSubmit = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        notify("অনুদান সফলভাবে সেভ করা হয়েছে!");
      }
    } finally { 
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 text-slate-900 relative">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <StatCard title="মোট অনুদান" amount={stats.totalDonation} icon={<TrendingUp />} color="emerald" />
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex p-1.5 bg-slate-200/50 rounded-[20px] w-full max-w-md shadow-inner border border-slate-200">
            <button onClick={() => setActiveForm('donation')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[15px] font-bold transition-all ${activeForm === 'donation' ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-500 hover:bg-slate-100'}`}>
              <PlusCircle size={18}/> অনুদান সংগ্রহ
            </button>
          ...
          <div className="lg:col-span-5">
            <div className={`bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-100 transition-all ${activeForm === 'donation' ? 'border-t-4 border-t-emerald-500' : 'border-t-4 border-t-red-500'}`}>
              <h2 className={`text-xl font-black mb-8 flex items-center gap-3 ${activeForm === 'donation' ? 'text-emerald-700' : 'text-red-700'}`}>
                ...
                {activeForm === 'donation' ? 'নতুন অনুদান রেকর্ড' : 'নতুন খরচ রেকর্ড'}
              </h2>
              ...
              <div className="space-y-5">
                {activeForm === 'donation' ? (
                  ...
                  <>
                    <input placeholder="খরচের খাত (যেমন: ঈদগাহ সংস্কার সামগ্রী) *" value={expense.receiverName} onChange={(e)=>setExpense({...expense, receiverName:e.target.value})} className="w-full p-4 bg-rose-50/30 border border-rose-100 rounded-2xl outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all" />
...
// StatCard and TransactionItem Components
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400 mt-1 font-medium">
            ...
            <span className="truncate max-w-[150px]">{transaction.note || (isDonation ? 'ঈদগাহ তহবিল' : 'খরচ')}</span>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-4">
