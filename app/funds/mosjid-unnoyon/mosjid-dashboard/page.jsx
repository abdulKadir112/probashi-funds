...
import { 
    FaArrowLeft, FaTrashAlt, FaWallet, FaArrowUp, FaArrowDown, 
    FaPlusCircle, FaMinusCircle, FaCheckCircle, FaExclamationCircle, 
    FaLock, FaUnlock, FaKey, FaUser, FaSignOutAlt, FaMosque, FaGift 
} from "react-icons/fa";
import { AiOutlineLoading3Quarters } from "react-icons/ai"; 
import Link from 'next/link';
import { useStore } from '../../../lib/store';

export default function MosjidDashboard() {
  const { transactions, fetchData, addTransaction, deleteTransaction } = useStore();
  
  const FUND_ID = 'mosjid-unnoyon';
  const OTHER_FUNDS = ['iftaar-tohobil', 'general-fund', 'asahay-sahajjo', 'mosjid-unnoyon']; 
  ...
  // Donation state থেকে receiver তথ্য বাদ দেওয়া হয়েছে
  const [donation, setDonation] = useState({ donorName: '', amount: '', note: '' });
  const [expense, setExpense] = useState({ receiverName: '', amount: '', note: '' });
  ...
  useEffect(() => {
    const savedAuth = localStorage.getItem('admin_auth_mosjid');
  ...
  const handleLogin = (e) => {
    ...
    if (inputPasskey === ADMIN_PASSKEY) {
      setIsAuthorized(true);
      localStorage.setItem('admin_auth_mosjid', ADMIN_PASSKEY);
      notify("মসজিদ ফান্ড ড্যাশবোর্ড আনলক হয়েছে!");
      loadAllData();
    } else {
      setError(true);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('admin_auth_mosjid');
    setInputPasskey('');
  };
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6">
          <div className={`bg-white rounded-[40px] p-10 max-w-sm w-full shadow-2xl border transition-all duration-300 ${error ? 'border-red-500 shake-animation' : 'border-slate-100'}`}>
            <div className="w-20 h-20 bg-emerald-900 text-white rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-xl">
              <FaLock size={35} />
            </div>
            <h2 className="text-2xl font-black text-slate-800 mb-2 text-center italic uppercase tracking-tighter">মসজিদ অ্যাডমিন</h2>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="relative">
                <FaKey className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${error ? 'text-red-500' : 'text-slate-300'}`} size={20} />
                <input 
                  type="password" 
                  placeholder="পাসকোড" 
                  autoFocus
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border placeholder:text-slate-300 text-slate-800 border-slate-200 rounded-2xl outline-none focus:border-emerald-600 transition-all font-black text-center text-lg tracking-[10px]" 
                />
              </div>
  
              <button 
                type="submit" 
                className="w-full py-4 bg-emerald-900 text-white rounded-2xl font-black shadow-lg active:scale-95 transition-all uppercase tracking-widest flex items-center justify-center gap-2"
              >
                আনলক <FaUnlock size={18} />
              </button>
            </form>
          </div>
          <style jsx>{`
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 lg:p-12 text-slate-900">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 bg-white/50 backdrop-blur-md p-6 rounded-[32px] border border-white shadow-sm">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <Link href={`/funds/${FUND_ID}`}>
              <button className="group flex items-center gap-2 bg-white border border-slate-200 px-5 py-3 rounded-2xl font-bold text-slate-600 hover:bg-emerald-900 hover:text-white hover:border-emerald-900 transition-all duration-300 shadow-sm active:scale-95">
          ...
          <div className="text-center flex-1">
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 italic uppercase tracking-tighter leading-none mb-1">
              মসজিদ <span className="text-emerald-600">উন্নয়ন</span>
            </h1>
            <div className="flex items-center justify-center gap-2">
              <span className="h-[2px] w-8 bg-slate-200"></span>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] pl-1">Mosque Admin Panel</p>
              <span className="h-[2px] w-8 bg-slate-200"></span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <StatCard title="মোট দান" amount={stats.totalDonation} icon={<FaArrowUp />} color="emerald" />
          <StatCard title="মোট খরচ" amount={stats.totalExpense} icon={<FaArrowDown />} color="red" />
          <StatCard title="ব্যব্যালেন্স" amount={stats.balance} icon={<FaWallet />} color="indigo" />
        </div>
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex p-1.5 bg-slate-200/50 rounded-[20px] w-full max-md:max-w-full max-w-md shadow-inner border border-white">
            <button onClick={() => setActiveForm('donation')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[15px] font-bold transition-all ${activeForm === 'donation' ? 'bg-white text-emerald-600 shadow-md scale-95' : 'text-slate-500'}`}>
              <FaPlusCircle size={18}/> দান সংগ্রহ
            </button>
            <button onClick={() => setActiveForm('expense')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[15px] font-bold transition-all ${activeForm === 'expense' ? 'bg-white text-red-600 shadow-md scale-95' : 'text-slate-500'}`}>
              <FaMinusCircle size={18}/> খরচ/ব্যয়
            </button>
          </div>
        </div>
        ...
        <div className="bg-white p-6 md:p-10 rounded-[40px] shadow-2xl border border-slate-100 relative">
          ...
          {activeForm === 'donation' ? <FaMosque size={24} /> : <FaArrowDown size={24} />}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeForm === 'donation' ? (
              <>
                    ...
                    {showSuggestions && filteredDonors.length > 0 && (
                        <div className="absolute z-[100] w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl max-h-48 overflow-y-auto">
                            {filteredDonors.map((name, index) => (
                                ...
                                >
                                    <FaUser size={14} className="text-emerald-500" />
                ...
                {/* Receiver Name ফিল্ডটি এখান থেকে সরানো হয়েছে */}
                ...
                <InputGroup label="নোট / উদ্দেশ্য" placeholder="যেমন: উদ্দেশ্য" value={donation.note} onChange={(v)=>setDonation({...donation, note:v})} />
              </>
            ) : (
              <>
                <InputGroup label="ব্যয়ের বিবরণ / গ্রহীতা *" placeholder="কার কাছে দেওয়া হলো" value={expense.receiverName} onChange={(v)=>setExpense({...expense, receiverName:v})} />
                ...
                <InputGroup label="খরচের উদ্দেশ্য" placeholder="যেমন: খরচের উদ্দেশ্য" value={expense.note} onChange={(v)=>setExpense({...expense, note:v})} />
              </>
            )}
          </div>
          ...
          <button 
            onClick={activeForm === 'donation' ? async () => {
              ...
              // ডিফল্টভাবে receiverName: 'মসজিদ' সেট করা হয়েছে
              ...
              if (success) {
                setDonation({ donorName: '', amount: '', note: '' });
                notify("দান সফলভাবে সেভ হয়েছে!");
          ...
          >
            {isSubmitting ? <AiOutlineLoading3Quarters className="animate-spin" /> : 'তথ্য সেভ করুন'}
          </button>
        </div>
        ...
        <div className="mt-16 mb-24">
          <div className="bg-white rounded-[40px] shadow-xl border border-slate-100 overflow-hidden">
            <div className="p-8 border-b bg-slate-50/50 flex justify-between items-center">
                <h3 className="font-black text-slate-800 italic uppercase text-xs tracking-widest underline underline-offset-4 decoration-emerald-500">সর্বশেষ লেনদেন (মসজিদ ফান্ড)</h3>
              ...
              {listData.map((t) => (
                <div key={t._id} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-all group animate-in fade-in duration-500">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setDeleteModal({ show: true, id: t._id })} className="w-10 h-10 flex items-center justify-center rounded-xl bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all"><FaTrashAlt size={18}/></button>
                  </div>
                </div>
              ))}
              {listData.length === 0 && (
                <div className="p-20 text-center text-slate-300 font-bold italic uppercase tracking-widest">কোন ডাটা পাওয়া যায়নি</div>
              )}
            </div>
          </div>
        </div>
